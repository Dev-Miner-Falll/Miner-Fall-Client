...
MediumBombStrategy::MediumBombStrategy()
    : ExplosiveItemStrategy(TILE_SIZE * 3, 3.f)
{}

void MediumBombStrategy::Execute(const ItemActionContext& context)
{
    ...
    explosive_item->Inject("Medium Bomb.png", context.m_pickaxe->getPosition(), m_radius, m_interval);
    explosive_item->Activate(context, 7.5f, 1.0f);
